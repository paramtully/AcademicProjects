...
template <template <class T> class OrderingStructure> animation filler::Fill(FillerConfig& config)
{
  ...
  */
  ...
  vector<vector<PixelPoint>> map;
  initMap(map, config);

  // cout << "STARTING FILL ON IMG -- WIDTH: " << config.img.width() << " x   HEIGHT: " << config.img.height() <<  ":" << endl; 

  os.Add(map[config.seedpoint.y][config.seedpoint.x]);
  map[config.seedpoint.y][config.seedpoint.x].color.a = 0;

  // for (unsigned int x = 0; x < config.img.width(); x++) {
  //   for (unsigned int y = 0; y < config.img.height(); y++) {
  //     PixelPoint pp = map[y][x];
  //     if (x != pp.x) cout << "x : " << x << "   xmap: " << pp.x << endl;
  //     if (y != pp.y) cout << "y : " << y << "   ymap: " << pp.y << endl;
  //   }
  // }
  ...
  while(!os.IsEmpty()) {
    ...
    // cout << "added pixel x: " << pp.x << "       y: " << pp.y << endl;

    HSLAPixel p = config.picker->operator()(pp);
    *config.img.getPixel(pp.x, pp.y) = p;

    addNeighbours(pp.x, pp.y, map, os, config);
...
void filler::addNeighbours(int x, int y, vector<vector<PixelPoint>> &map, OrderingStructure<PixelPoint> &os, FillerConfig &config) {
  //                                    UP     DOWN     LEFT    RIGHT
  static vector<pair<int, int>> pairs{{0, 1},{0, -1}, {-1, 0}, {1, 0}};
  ...
  for (pair<int, int> p: pairs) {
    ...
    if (isWithinImage(cx, cy, config) && !isVisited(cx, cy, map) && isWithinTolerance(cx, cy, config)) {
      // cout << "adding neighbour " << (p == pairs[0]? "UP" : p == pairs[1]? "DOWN" : p == pairs[2]? "LEFT" : "RIGHT")  << ":  x: " << cx << "    y: " << cy << endl;
      config.neighbourorder.Insert(map[cy][cx]);
      map[cy][cx].color.a = 0;
    }
  }
...
void filler::initMap(vector<vector<PixelPoint>> &map, FillerConfig &config) {
  map.resize(config.img.height());

  for (unsigned int y = 0; y < config.img.height(); y++) {
    for (unsigned int x = 0; x < config.img.width(); x++) {
      PixelPoint pp(x, y, *config.img.getPixel(x, y));
      map[y].push_back(pp);
    }
  }
}

bool filler::isVisited(int x, int y, vector<vector<PixelPoint>> &map) {
  return map[y][x].color.a == 0;
}

bool filler::isWithinImage(int x, int y, FillerConfig &config) {
  return x >= 0 && x < (int)config.img.width() && y >= 0 && y < (int)config.img.height();
}

bool filler::isWithinTolerance(int x, int y, FillerConfig &config) {
  return config.img.getPixel(x, y)->dist(config.seedpoint.color) <= config.tolerance;
}
