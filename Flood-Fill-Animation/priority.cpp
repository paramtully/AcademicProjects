...
PixelPoint PriorityNeighbours::Remove() {
  ...
  for (int i = 0; i < size; i++) {
    ...
    else if (isPixelCloser(points[i], points[min_pos])) min_pos = i;
  }
  PixelPoint min_point = points[min_pos];
  points.erase(points.begin() + min_pos);
  return min_point;
}

bool PriorityNeighbours::isPixelCloser(PixelPoint p, PixelPoint min_point) {
  return p.color.dist(refcolor) < min_point.color.dist(refcolor) ||
    (p.color.dist(refcolor) == min_point.color.dist(refcolor) && p.y < min_point.y) ||
    (p.y == min_point.y && p.x < min_point.x);
} 
