...
{
  // complete your implementation below
  delete [] items;                                // CORRECTION: arrays/pointers need internal items deleted via delete []
...
T Stack<T>::Pop() {
  ...
  if (num_items < max_items / SHRINKRATE && newSize >= DEFAULTCAPACITY) 
    Resize(newSize);
  return item; 
};
...
void Stack<T>::Resize(size_t n) {
  ...
  delete [] s;
  max_items = n;                          // CORRECTION: didnt update max_items to new max_size
};                                        //             (buffer overflow)
