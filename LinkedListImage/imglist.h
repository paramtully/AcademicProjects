...
class ImgList {
  ...
  *********************************************************/
  private:
    ...
    void createSingleRow(int x, ImgNode *&node, PNG& img);
    void createRows(unsigned int x, unsigned int y, ImgNode *&node, PNG& img);
    void createColumnPair(unsigned int x, unsigned int y, ImgNode *&top, ImgNode *&right, PNG& img);
    void linkEastPairHorizontally(ImgNode *&top, ImgNode *&bottom);
    void linkPairVertically(ImgNode *&top, ImgNode *&bottom);

    // helpers for GetDimensionX(), GetDimensionFullX(), and GetDimensionY()
    unsigned int countX(ImgNode *node) const;
    unsigned int countY(ImgNode *node) const;
    unsigned int countFullX(ImgNode *node) const;

    // helpers for Render()
    int setPixel(int x, int y, ImgNode *&node, PNG &png, bool fillgaps, int fillmode) const;

    // helpers for Cleave()
    void detatchHorizontally(ImgNode *&toRemove);
    void detatchNorth(ImgNode *&toRemove);
    void detatchSouth(ImgNode *&toRemove);

    // helpers for Clear()
    void clearColumn(ImgNode *node);
    void clearRow(ImgNode *node);
};
