...
void testRender0();
void testRender1();
void testRenderFillGap6x6();
void testMemLeak();

int ImgList::n = 0; int ImgList::d = 0;
...
void TestRenderMode2() {
  ...
  if (inimg.height() == 6 && inimg.width() == 6) {
    ...
    PNG ans;
    ans.readFromFile("output-images/soln-6x6-1-2.png");
    cout << endl;

    for (int i = 0; i < inimg.height(); i++) {
      for (int j = 0; j < inimg.width(); j++) {
        HSLAPixel *p = ans.getPixel(j, i);
        HSLAPixel *c = outputimg2.getPixel(j, i);
        cout << "x: " << j << ", y: " << i << endl;
...
void testRender0() {
  ...
  PNG inimg;
  inimg.readFromFile("output-images/soln-5x6-1-0.png");
  PNG outimg;
  outimg.readFromFile("output-images/5x6-1-0.png");

  for (int i = 0; i < inimg.height(); i++) {
    for (int j = 0; j < inimg.width(); j++) {
      HSLAPixel *p = inimg.getPixel(j, i);
      HSLAPixel *c = outimg.getPixel(j, i);
      cout << "x: " << j << ", y: " << i << endl;
      cout << "h: " << p->h << ", copy h: "  << c->h << endl;
      cout << "s: " << p->s << ", copy s: "  << c->s << endl;
      cout << "l: " << p->l << ", copy l: "  << c->l << endl;
      cout << "a: " << p->a << ", copy a: "  << c->a << endl << endl;
    }
  }
}
...
void testRender1() {
  ...
  PNG inimg;
  inimg.readFromFile("output-images/soln-5x6-1-1.png");
  PNG outimg;
  outimg.readFromFile("output-images/5x6-1-1.png");
...
void testRenderFillGap6x6() {
  ...
  cout << "FullX: " << l.GetDimensionFullX() << endl << endl;
  ImgNode* node = l.GetNorthwest();
  int z = 0;
  while (node) {
    cout << "Row " << z << " skipright = " << node->skipright << endl;

    node = node->south;
  }
  cout << endl;

  PNG outimg = l.Render(true, 0);
  ...
  for (int i = 0; i < inimg.height(); i++) {
    for (int j = 0; j < inimg.width(); j++) {
      ...
      // NOTE: left and right should be different
      //       expect every pixel in row to be same except last
      cout << "x: " << j << ", y: " << i << endl;
      cout << "h: " << p->h << ", copy h: "  << c->h << endl;
      cout << "s: " << p->s << ", copy s: "  << c->s << endl;
      cout << "l: " << p->l << ", copy l: "  << c->l << endl;
...
void testMemLeak() {
    ...
    cout << "Num new: " << ImgList::n << endl;
    cout << "Num del: " << ImgList::d << endl;
