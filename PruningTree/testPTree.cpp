...
// TEST_CASE("PTree::Constructor(1x1)") {
//   cout << endl;
//   cout << "-------------------------------------------------------------------------------" << endl;
//   cout << "PTree::Constructor (1x1)" << endl;
//   cout << "-------------------------------------------------------------------------------" << endl << endl;

//   PNG opng(1, 1);
//   *opng.getPixel(0, 0) = HSLAPixel(5, 0.6, 0.6, 0.6);
//   PTree otree(opng);
//   HSLAPixel op = otree.GetRoot()->avg;
//   cout << "n: " << otree.Size() << endl;
//   cout << "h: " << op.h << endl;
//   cout << "s: " << op.s << endl;
//   cout << "l: " << op.l << endl;
//   cout << "a: " << op.a << endl;
...
TEST_CASE("PTree::Constructor(1x2)") {
  cout << endl;
  cout << "-------------------------------------------------------------------------------" << endl;
  cout << "PTree::Constructor (1x2)" << endl;
  cout << "-------------------------------------------------------------------------------" << endl << endl;
  
  PNG opng(1, 2);
  *opng.getPixel(0, 0) = HSLAPixel(5, 0.6, 0.6, 0.6);
  *opng.getPixel(0, 1) = HSLAPixel(5, 0.6, 0.6, 0.6);
  PTree otree(opng);
  HSLAPixel op = otree.GetRoot()->avg;
  cout << "n: " << otree.Size() << endl;
  cout << "h: " << op.h << endl;
...
TEST_CASE("PTree::Constructor", "[weight=1][part=ptree]") {
  ...
  *px01 = HSLAPixel(350, 0.8, 0.2);    *px11 = HSLAPixel(240, 0.1, 0.8);     *px21 = HSLAPixel(320, 0.7, 0.4);
  ...
  cout << endl;
  cout << "-------------------------------------------------------------------------------" << endl;
  cout << "PTree::Constructor (Check Children)" << endl;
  cout << "-------------------------------------------------------------------------------" << endl << endl;
  ...
  // prints leaves
  // while (!stack.empty()) {
  //   Node *cNode = stack.top(); stack.pop();
  //   if (cNode) {
  //     stack.push(cNode->B);
  //     stack.push(cNode->A);
  //     if (!cNode->A && !cNode->B) {
  //       cout << "h: " << cNode->avg.h << " s: " << cNode->avg.s << " l: " << cNode->avg.l << endl;
  //     }
  //   }
  // }
  ...
  //   Node *cNode = stack.top(); stack.pop();
  //   if (cNode) {
  //     if (!cNode->A && !cNode->B) cout << "Child: " << endl;
  //     cout << "h: " << cNode->avg.h << " s: " << cNode->avg.s << " l: " << cNode->avg.l << endl;
  //     if(cNode->A) cout << "Adding A" << endl;
  //     if(cNode->B) cout << "Adding B" << endl;
  //     stack.push(cNode->B);
  //     stack.push(cNode->A);
  //   }
  // }
  ...
  cout << " l: " << avgl << endl;     // ADDED

  REQUIRE(nd->avg == HSLAPixel(avgh, avgs, avgl));

  // root->A
  cout << "Expected root->A: ";
  nd = nd_root->A;
  REQUIRE(nd->upperleft == make_pair((unsigned int)0, (unsigned int)0));
  REQUIRE(nd->width == 1);
  REQUIRE(nd->height == 2);
